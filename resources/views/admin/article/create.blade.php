                    ...
                    <div class="col-12">
                        <form action="{{ route('admin.article.store') }}" method="POST">
                            ...
                            <div class="form-group  w-25">
                                <input type="text" class="form-control" name="article" placeholder="Введите артикул" value="{{ old('article') }}">
                                @error('article')
                            ...
                            <div class="form-group w-25">
                                ...
                                <select name="floor_id" class="form-control">
                                    ...
                                    <option value="{{ $floor->id }}" {{ $floor->id == old('$floor_id') ? ' selected' : '' }}>{{ $floor->floor }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group w-25">
                                ...
                                <select class="select2" name="place_ids[]" multiple="multiple" data-placeholder="Добавьте место" style="width: 100%;">
                                    @foreach($places as $place)
                                        <option {{ is_array(old('place_ids')) && in_array($place->id, old('place_ids')) ? ' selected' : '' }} value="{{ $place->id }}">{{ $place->place }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group">
